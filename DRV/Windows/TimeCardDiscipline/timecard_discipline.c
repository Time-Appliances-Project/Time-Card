            ...
            uint32_t error_length)
{
    ...
    config.oscillator_factory_settings = true;
    config.learn_temperature_table = false;
    config.use_temperature_table = false;
    ...
    if (saved_parameters != NULL) {
        ...
        if (candidate && saved.dsc_config.header == HEADER_MAGIC &&
            saved.dsc_config.version == DISCIPLINING_CONFIG_VERSION &&
            saved.temp_table.header == HEADER_MAGIC &&
            saved.temp_table.version == DISCIPLINING_CONFIG_VERSION) {
...
tcod_process(void *context, const tcod_input *input, tcod_output *output)
{
    ...
    algorithm_input.phasemeter_status = algorithm_input.valid ?
        PHASEMETER_BOTH_TIMESTAMPS : PHASEMETER_NO_GNSS_TIMESTAMPS;
