...
$invariants = @(
    ...
    @($driver, 'TimeCardReadCapturedPhase[\s\S]*before\s*==\s*after',
        'coherent interrupt-captured timestamp latching'),
    @($driver, 'PhaseReference->InterruptMask,[\s\S]{0,80}1u[\s\S]{0,160}PhaseOscillator->InterruptMask,[\s\S]{0,80}1u',
        'paired phase capture with both interrupts enabled'),
    @($driver, 'TimeCardPollPhaseTimestampLocked[\s\S]*reg->Interrupt,\s*1u',
        'ART polling fallback for bitstreams without MSI-X routing'),
